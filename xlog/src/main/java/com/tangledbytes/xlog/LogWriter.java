...
public final class LogWriter {
    ...
    public static StackTraceElement getStackTraceElement() {
        return ___some_random_name_it_must_be_unique_name__dfjfdsleuruewir489375fdjklasjfkjacndnbfjker70987654321();
    }
     ...
     */
    private static StackTraceElement ___some_random_name_it_must_be_unique_name__dfjfdsleuruewir489375fdjklasjfkjacndnbfjker70987654321() {
    ...
    public void addLog(int priority, String tag, String msg, Throwable tr) {
        ...
        try {
            ...
            // Parse tag
            if (tag.length() > 20)
                tag = tag.substring(0, 9) + ".." + tag.substring(tag.length() - 9);
            else if (tag.length() < 20) {
                StringBuilder tagBuilder = new StringBuilder(tag);
                while (tagBuilder.length() != 20)
                    tagBuilder.append(" ");
